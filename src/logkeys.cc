...
void determine_input_device(std::vector<std::string>& results)
{
...
#define TIME_FORMAT "%F %T%z > "  // results in YYYY-mm-dd HH:MM:SS+ZZZZ
  ...
  for(unsigned long i = 0; i < results.size(); i++) {
    int fd = open(results[i].c_str(), O_RDONLY);
    if(fd < 0){
      error(EXIT_FAILURE, errno, "Error opening input event device '%s'", results[i].c_str());
    }
...
#define EV_REPEAT 2  // when key switches to repeating after short delay
    ...
    }// for(unsigned long i = 0; i < fds.size() && x > 0; i++)
    FD_ZERO(&read_fds);
    for(unsigned long i = 0; i < input_fds.size() && x > 0; i++){
      int fd = input_fds[i];
      FD_SET(fd, &read_fds);
